...
NEXT_BEARING = {
    "send_message", "ask_question", "set_var", "branch",
    "send_photo", "send_document", "buttons", "delay",
}
...
def validate_graph(graph: dict[str, Any]) -> list[Issue]:
    ...
    for n in nodes:
        ...
        if not nid:
            issues.append({"level": "error", "node_id": None,
                           "code": "missing_id", "message": "Узел без id"})
            continue
    ...
    if not has_trigger:
        issues.append({"level": "error", "node_id": None, "code": "no_trigger",
    ...
    for nid in node_by_id:
        ...
        if nid not in reachable:
            issues.append({"level": "hint", "node_id": nid, "code": "unreachable",
